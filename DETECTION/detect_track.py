...
def classify_jersey_hsv(crop):
    ...
    # frames (in run_pipeline) correct it, rather than defaulting to referee.
...
# =====================================================================
def run_pipeline(input_video_path, mock_mode=False):
    print("=" * 60)
    print("SmartMatch AI Tracker Pipeline Launching")
    print("=" * 60)
    mock_tracks_data = None
    if mock_mode:
        print("[Mode] Running in synthetic MOCK mode...")
        # Create output directory structure if missing
        os.makedirs("outputs", exist_ok=True)
        # Generate the synthetic match video
        ...
        if not os.path.exists(input_video_path):
            ...
            print("To run a demo, execute the script with the '--mock' flag.")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    run_pipeline(args.input, mock_mode=args.mock)